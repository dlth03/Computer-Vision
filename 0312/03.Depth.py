import cv2                   # OpenCV 라이브러리
import numpy as np           # 배열 처리를 위한 NumPy 라이브러리 불러오기
from pathlib import Path     # 파일 경로 관리용

# 출력 폴더 생성
output_dir = Path("./outputs")
output_dir.mkdir(parents=True, exist_ok=True)

# 좌/우 이미지 불러오기
left_color = cv2.imread("left.png")
right_color = cv2.imread("right.png")

# 이미지가 없으면 오류 발생
if left_color is None or right_color is None:
    raise FileNotFoundError("좌/우 이미지를 찾지 못했습니다.")

# 카메라 파라미터
f = 700.0
B = 0.12

# ROI 설정
rois = {
    "Painting": (55, 50, 130, 110),
    "Frog": (90, 265, 230, 95),
    "Teddy": (310, 35, 115, 90)
}

# -----------------------------
# 그레이스케일 변환
# -----------------------------
left_gray = cv2.cvtColor(left_color, cv2.COLOR_BGR2GRAY)     # 좌 이미지 그레이스케일
right_gray = cv2.cvtColor(right_color, cv2.COLOR_BGR2GRAY)   # 우 이미지 그레이스케일

# -----------------------------
# 1. Disparity 계산
# -----------------------------
stereo = cv2.StereoBM_create(numDisparities=128, blockSize=15)   # StereoBM 객체 생성

disparity = stereo.compute(left_gray, right_gray).astype(np.float32)
disparity = disparity / 16.0   # OpenCV는 16배 스케일로 반환

valid_mask = disparity > 0   # 유효한 disparity 픽셀 마스크

# -----------------------------
# 2. Depth 계산
# Z = fB / d
# -----------------------------
depth_map = np.zeros_like(disparity, dtype=np.float32)

depth_map[valid_mask] = (f * B) / disparity[valid_mask]     # 유효 픽셀만 계산

# -----------------------------
# 3. ROI별 평균 disparity / depth 계산
# -----------------------------
results = {}

for name, (x, y, w, h) in rois.items():
    disp_roi = disparity[y:y+h, x:x+w]
    depth_roi = depth_map[y:y+h, x:x+w]

    valid_roi = disp_roi > 0  # ROI 내 유효 픽셀만

    if np.any(valid_roi):
        mean_disp = np.mean(disp_roi[valid_roi])
        mean_depth = np.mean(depth_roi[valid_roi])
    else:
        mean_disp = np.nan
        mean_depth = np.nan

    results[name] = {
        "mean_disparity": mean_disp,
        "mean_depth": mean_depth
    }

# -----------------------------
# 4. 결과 출력
# -----------------------------
print("\n===== ROI Results =====")

for name, val in results.items():
    print(f"{name}")
    print(f"  Mean Disparity: {val['mean_disparity']:.2f}")
    print(f"  Mean Depth: {val['mean_depth']:.2f}")

# 가장 가까운 / 먼 ROI
closest = min(results.items(), key=lambda x: x[1]['mean_depth'])
farthest = max(results.items(), key=lambda x: x[1]['mean_depth'])

print("\n===== Distance Analysis =====")
print(f"Closest ROI : {closest[0]}")
print(f"Farthest ROI: {farthest[0]}")

# -----------------------------
# 5. disparity 시각화
# 가까울수록 빨강 / 멀수록 파랑
# -----------------------------
disp_tmp = disparity.copy()
disp_tmp[disp_tmp <= 0] = np.nan  # 유효하지 않은 값 제거

if np.all(np.isnan(disp_tmp)):
    raise ValueError("유효한 disparity 값이 없습니다.")

d_min = np.nanpercentile(disp_tmp, 5)  # 하위 5% 제외
d_max = np.nanpercentile(disp_tmp, 95) # 상위 5% 제외

if d_max <= d_min:
    d_max = d_min + 1e-6

disp_scaled = (disp_tmp - d_min) / (d_max - d_min)
disp_scaled = np.clip(disp_scaled, 0, 1)

disp_vis = np.zeros_like(disparity, dtype=np.uint8)
valid_disp = ~np.isnan(disp_tmp)
disp_vis[valid_disp] = (disp_scaled[valid_disp] * 255).astype(np.uint8)

disparity_color = cv2.applyColorMap(disp_vis, cv2.COLORMAP_JET)  # 컬러맵 적용

# -----------------------------
# 6. depth 시각화
# 가까울수록 빨강 / 멀수록 파랑
# -----------------------------
depth_vis = np.zeros_like(depth_map, dtype=np.uint8)

if np.any(valid_mask):
    depth_valid = depth_map[valid_mask]

    z_min = np.percentile(depth_valid, 5)
    z_max = np.percentile(depth_valid, 95)

    if z_max <= z_min:
        z_max = z_min + 1e-6

    depth_scaled = (depth_map - z_min) / (z_max - z_min)
    depth_scaled = np.clip(depth_scaled, 0, 1)   # 가까울수록 빨강

    depth_scaled = 1.0 - depth_scaled
    depth_vis[valid_mask] = (depth_scaled[valid_mask] * 255).astype(np.uint8)

depth_color = cv2.applyColorMap(depth_vis, cv2.COLORMAP_JET)

# -----------------------------
# 7. Left / Right 이미지에 ROI 표시
# -----------------------------
left_vis = left_color.copy()
right_vis = right_color.copy()

for name, (x, y, w, h) in rois.items():
    cv2.rectangle(left_vis, (x, y), (x + w, y + h), (0, 255, 0), 2)
    cv2.putText(left_vis, name, (x, y - 8),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

    cv2.rectangle(right_vis, (x, y), (x + w, y + h), (0, 255, 0), 2)
    cv2.putText(right_vis, name, (x, y - 8),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

# -----------------------------
# 8. 저장
# -----------------------------
cv2.imwrite(str(output_dir / "left_roi.png"), left_vis)
cv2.imwrite(str(output_dir / "right_roi.png"), right_vis)
cv2.imwrite(str(output_dir / "disparity.png"), disparity_color)
cv2.imwrite(str(output_dir / "depth.png"), depth_color)

# -----------------------------
# 9. 출력
# -----------------------------
print("\nImages saved to ./outputs")